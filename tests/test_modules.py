...
class TestModules(unittest.TestCase):
    def setUp(self):
        ...
        if os.path.exists("lora_model_test"):
            shutil.rmtree("lora_model_test")
        if os.path.exists("quantized_model_test"):
            shutil.rmtree("quantized_model_test")
        if os.path.exists("onnx_model_test"):
            shutil.rmtree("onnx_model_test")
    ...
    def test_drift_detector(self):
        ...
        reference_data = pd.DataFrame(
            {"feature1": [1, 2, 3, 4, 5], "feature2": [6, 7, 8, 9, 10]}
        )
        new_data = pd.DataFrame(
            {"feature1": [11, 12, 13, 14, 15], "feature2": [16, 17, 18, 19, 20]}
        )
